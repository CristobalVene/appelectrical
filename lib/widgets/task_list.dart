    ...
    return StreamBuilder<List<Project>>(
      ...
      builder: (context, projectSnapshot) {
        ...
        if (projectId != null) {
          ...
          projectIdsToFilter = allProjects.where((p) => p.userIds.contains(currentUserId)).map((p) => p.id).toList();
        }

        if (projectIdsToFilter.isEmpty) {
          return const Center(child: Text("You are not assigned to any projects."));
        }
        ...
        return StreamBuilder<List<Task>>(
          ...
          builder: (context, taskSnapshot) {
            ...
            if (!taskSnapshot.hasData || taskSnapshot.data!.isEmpty) {
              return const Center(child: Text("No tasks found for your projects."));
            }

            final filteredTasks = taskSnapshot.data!.where((task) => projectIdsToFilter.contains(task.projectId)).toList();
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final projectName = allProjects.firstWhere((p) => p.id == task.projectId, orElse: () => Project(id: '', name: 'Unknown', userIds: [])).name;
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            showDialog(context: context, builder: (context) => Dialog(child: TaskForm(projectId: task.projectId)));
                          },
                        ),
                        IconButton(
